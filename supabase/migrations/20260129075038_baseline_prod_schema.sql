...
CREATE OR REPLACE FUNCTION "public"."get_top_weighted_terms"("limit_count" integer DEFAULT 15) RETURNS TABLE("slug" "text", "term" "text")
    LANGUAGE "sql" STABLE
    AS $$
  select 
    t.slug,
  ...
  order by max(d.weight) desc, t.term
  limit limit_count;
$$;


ALTER FUNCTION "public"."get_top_weighted_terms"("limit_count" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."search_terms"("search_query" "text", "result_limit" integer DEFAULT 20) RETURNS TABLE("slug" "text", "term" "text", "similarity_score" real)
    LANGUAGE "sql" STABLE
    AS $$
  select 
    t.slug,
    t.term,
    -- Calculate similarity score (1.0 for exact match, lower for partial)
    case 
      when lower(t.term) = lower(search_query) then 1.0
      when lower(t.term) like lower(search_query) || '%' then 0.9
      when lower(t.term) like '%' || lower(search_query) || '%' then 0.7
      else similarity(t.term, search_query)
    end as similarity_score
  from public.terms t
  where 
    exists (
      select 1 from public.definitions d 
      where d.term_id = t.id and d.status = 'published'
    )
    and (
      lower(t.term) like '%' || lower(search_query) || '%'
      or t.term % search_query  -- trigram similarity
    )
  order by similarity_score desc, t.term
  limit result_limit;
$$;


ALTER FUNCTION "public"."search_terms"("search_query" "text", "result_limit" integer) OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."definitions" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "definitions_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'pending'::"text", 'published'::"text", 'rejected'::"text"])))
);
...
CREATE OR REPLACE TRIGGER "set_definitions_updated_at" BEFORE UPDATE ON "public"."definitions" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
CREATE OR REPLACE TRIGGER "set_terms_updated_at" BEFORE UPDATE ON "public"."terms" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
GRANT ALL ON FUNCTION "public"."get_top_weighted_terms"("limit_count" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_top_weighted_terms"("limit_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_top_weighted_terms"("limit_count" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."search_terms"("search_query" "text", "result_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."search_terms"("search_query" "text", "result_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."search_terms"("search_query" "text", "result_limit" integer) TO "service_role";
